...
namespace ERPAPI.Controllers
{
    ...
    public class PurchController : Controller
    {
        ...
        /// <summary>
        /// Obtiene el Listado de Purch paginado
        /// </summary>
        /// <returns></returns>    
        [HttpGet("[action]")]
        public async Task<IActionResult> GetPurchPag(int numeroDePagina = 1, int cantidadDeRegistros = 20)
        {
            List<Purch> Items = new List<Purch>();
            try
            {
                var query = _context.Purch.AsQueryable();
