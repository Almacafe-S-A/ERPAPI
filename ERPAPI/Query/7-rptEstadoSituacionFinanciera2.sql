...
/****** Object:  StoredProcedure [dbo].[GenerarEstadoSituacionFinanciera2]    Script Date: 16/1/2020 16:58:58 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GenerarEstadoSituacionFinanciera2]
	@ANIO INT,	
	@NIVEL INT,
	@CENTROCOSTO BIGINT
AS
BEGIN		
	...
	SELECT 
	Nota = CASE WHEN T4.ParentAccountId IS NULL THEN NULL ELSE ROW_NUMBER() OVER (PARTITION BY CASE WHEN T4.ParentAccountId IS NULL THEN 1 ELSE 0 END ORDER BY B.AccountId) END + 3,
